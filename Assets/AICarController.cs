...
public class AICarController : MonoBehaviour
{
    ...
    public float brakingSensitivity = 3.5f;           // Higher = brakes earlier/harder before turns
    ...
    public float brakeLookAheadDistance = 80f;  // fixed metres, independent of speed
    ...
    public float minSegmentSharpness = 0.025f; // ≈ 18° per segment — below this = ignored
    ...
    void FixedUpdate()
    {
        ...
        else if (finalSharpness > 0f)
        {
            ...
            // How strongly to respond: scaled by speed and sensitivity
            float response = finalSharpness * speedFactor * brakingSensitivity;

            // Stage 1: lift off throttle (response 0 → 0.6 maps throttle 1 → 0)
            // Threshold raised to 0.6 so distant-corner sharpness doesn't bleed
            // into throttle reduction — only genuinely close corners trigger lift-off.
            float liftOff = Mathf.Clamp01(response / 0.6f);
            throttleInput = Mathf.Lerp(throttleLimit, 0f, liftOff);

            // Stage 2: apply brakes (response 0.6 → 1.4 maps brakes 0 → -0.9)
            if (response > 0.6f)
            {
        ...
        if (reverseTimer > 0f)
        {
            ...
            // Steer opposite to target direction so the rear swings away from the wall
            float steerDirection = localTarget.x > 0f ? -0.8f : 0.8f;
            carController.aiSteerInput = steerDirection;
